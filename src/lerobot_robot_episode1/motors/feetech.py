...
# (import 改为包内相对路径;删掉 mock 分支里对 `tests.motors.mock_scservo_sdk` 的 import——
#  那是 fork 内部测试残留,在本包里是坏的。测试请在 FeetechController 层注入 fake。)
...
class FeetechMotorsBus:
    ...
    def connect(self):
        ...
        if self.mock:
            # mock 分支在 fork 里 import `tests.motors.mock_scservo_sdk`,那是 lerobot 仓内部
            # 测试残留,搬到插件包后是坏的,故删除。要测试请在 FeetechController 层注入 fake。
            raise NotImplementedError(
                "mock mode is not supported by the ported FeetechMotorsBus; "
                "inject a fake motors bus at the FeetechController level instead."
            )
