...
# (对 EnpeiLeader 的 import 改为从本包 constants 取夹爪量程常量)
...
# 夹爪量程从本包常量取（即 fork 里 EnpeiLeader.max/min_gripper_angle 的值，单一出处）
from lerobot_robot_episode1.constants import (
    DEFAULT_GRIPPER_MAX_ANGLE as MAX_GRIPPER_ANGLE,
    DEFAULT_GRIPPER_MIN_ANGLE as MIN_GRIPPER_ANGLE,
)
        ...
        for frame_idx in range(from_idx, to_idx):
            ...
            for key, value in frame.items():
                ...
                if key == "observation.state" or key == "action":
                    ...
                    if len(value) >= 7:
                        # 使用本包常量中的夹爪角度范围，保持一致性
                        # 与 episode1_follower.py 中的转换逻辑相同
                        
                        # 将[0,1]范围的值转换回角度范围
                        converted_value[6] = value[6] * (MAX_GRIPPER_ANGLE - MIN_GRIPPER_ANGLE) + MIN_GRIPPER_ANGLE
